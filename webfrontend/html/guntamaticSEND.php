...
<?php
    ...
    // Creates a log object, automatically assigned to your plugin, with the group name "GuntamaticLog"
    ...
    {
    	LOGCRIT("ERROR received from Guntamatic - unnown  command");
    ...
    // //LOGOK("Data received from Guntamatic:".json_encode($mowerlist));

    // // Kommando vomMiniserver erhalten welches an Guntamatic weitergeleitet werden muss
    // // Kommandos: "park", "pause", "start3h"

    // LOGOK("Send command '".$cmd."' to Guntamatic");

    // if(strpos($cmd,"cuttingHeight:")!==false) {
    //     $res = $session_guntamatic->settings($mowerID,array("cuttingHeight"=> intval(preg_replace('/[^0-9]/', '', $cmd))));
    // }
    ...
    // }
    // else 
    $res= $session_guntamatic->control($cmd);
    ...
    else LOGCRIT("ERROR received from Guntamatic".json_encode ($res));
